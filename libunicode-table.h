/*
 * libunicode-table.h - 压缩的 Unicode 数据表
 * 
 * 本文件包含 QuickJS Unicode 支持的核心数据表，采用压缩格式存储以减小内存占用
 * 这些表由 unicode_gen.c 工具自动生成，请勿手动编辑
 *
 * 数据表说明：
 * ============
 * 
 * 1. case_conv_table1/case_conv_table2 (大小写转换表)
 *    - 用于 Unicode 字符的大小写转换（大写↔小写↔标题大写）
 *    - 采用两级索引压缩：table1 存储主索引，table2 存储偏移量
 *    - 每个条目编码了字符范围、转换类型和目标偏移
 *    - case_conv_ext 存储扩展转换规则（如希腊语带重音字符）
 *
 * 2. unicode_prop_*_table (Unicode 属性表)
 *    - 存储各种 Unicode 字符属性的压缩位图
 *    - 使用游程编码 (RLE) 压缩连续相同值的区域
 *    - 每个表对应一个 Unicode 属性（如 Cased、Case_Ignorable、ID_Start 等）
 *    - 配套 *_index 数组提供快速索引定位
 *
 * 压缩格式说明：
 * =============
 * - 高位字节表示游程长度（重复次数）
 * - 低位字节表示属性值（0/1 或更复杂的编码）
 * - 特殊标记位表示索引跳转或扩展数据
 *
 * 文件大小：约 40KB（启用 CONFIG_ALL_UNICODE 时）
 * 数据来源：Unicode 15.0 标准
 */
...
/* ==========================================================================
 * 大小写转换主表 (case_conv_table1)
 * ==========================================================================
 * 存储 Unicode 字符大小写转换的核心索引数据
 * 数组大小：378 个条目
 * 数据格式：32 位编码，包含字符范围起始、长度和转换类型
 * 配合 case_conv_table2 使用，实现紧凑的转换规则存储
 */
...
/* ==========================================================================
 * 大小写转换辅助表 (case_conv_table2)
 * ==========================================================================
 * 存储大小写转换的偏移量和类型信息
 * 数组大小：378 个条目（与 table1 一一对应）
 * 数据格式：8 位编码，表示转换偏移或特殊规则索引
 * 与 table1 配合使用：table1 定位字符范围，table2 提供转换规则
 */
...
/* ==========================================================================
 * 大小写转换扩展表 (case_conv_ext)
 * ==========================================================================
 * 存储特殊情况的大小写转换规则（一个字符映射到多个字符）
 * 数组大小：58 个条目
 * 数据格式：16 位 Unicode 码点序列
 * 典型用例：
 * - 德语 ß (U+00DF) 大写化 → "SS" (两个字符)
 * - 希腊语带重音字符的特殊转换
 * - 连字字符（如 ﬁ → FI）的分解
 */
...
/* ==========================================================================
 * Unicode 属性表：Cased (有大小写属性)
 * ==========================================================================
 * 判断字符是否具有大小写（即是否为大写或小写字母）
 * 用于 JavaScript 的 toUpperCase/toLowerCase 等方法
 * 
 * unicode_prop_Cased1_table: 压缩的位图数据（190 字节）
 * - 使用游程编码 (RLE) 压缩
 * - 每个字节编码一段连续字符范围的属性值
 * 
 * unicode_prop_Cased1_index: 索引表（18 字节）
 * - 提供快速定位到 table 中特定 Unicode 区段的偏移
 * - 每 3 字节一个索引条目：2 字节码点前缀 + 1 字节 table 偏移
 */
...
/* Unicode 属性 Cased 的索引表
 * 格式：[高位字节，中位字节，低位字节] = [码点前缀高 8 位，码点前缀低 8 位，table 偏移]
 * 作用：根据 Unicode 码点快速定位到 table 中的压缩数据位置
 * 覆盖范围：U+0000 ~ U+1F18A（基本多文种平面 + 部分辅助平面）*/
static const uint8_t unicode_prop_Cased1_index[18] = {
    0xb9, 0x02, 0x80,  //  U+002B9 起始，偏移至 table[36]
    0xa0, 0x1e, 0x40,  //  U+01EA0 起始，偏移至 table[66]
    0x9e, 0xa6, 0x40,  //  U+0A69E 起始，偏移至 table[98]
    0x55, 0xd4, 0x21,  //  U+1D455 起始，偏移至 table[129]（数学字母符号）
    0x15, 0xd7, 0x21,  //  U+1D715 起始，偏移至 table[161]（数学数字）
    0x8a, 0xf1, 0x01,  //  U+1F18A 结束边界（索引表上限）
};

/* ==========================================================================
 * Unicode 属性表：Case_Ignorable (可忽略大小写)
 * ==========================================================================
 * 判断字符在大小写转换时是否应该被忽略
 * 用于 Unicode 大小写折叠算法（case folding）
 * 
 * 典型字符类型：
 * - 组合标记（Combining Marks）：如重音符号 U+0300-U+036F
 * - 零宽字符：如 ZWNJ (U+200C)、ZWJ (U+200D)
 * - 某些标点符号和格式控制符
 * 
 * 数组大小：785 字节（压缩后的游程编码数据）
 * 用途：正则表达式不区分大小写匹配、字符串比较等
 */
